...
exports.createQueue = function(namespace){
  redis.setNamespace(namespace);
  events.key = redis.key(events.key);
...
Queue.prototype.promote = function(ms){
  ...
  setInterval(function(){
    client.sort(redis.key('q:jobs:delayed')
      , 'by', redis.key('q:job:*->delay')
      , 'get', '#'
      , 'get', redis.key('q:job:*->delay')
      , 'get', redis.key('q:job:*->created_at')
...
Queue.prototype.setting = function(name, fn){
  this.client.hget(redis.key('q:settings'), name, fn);
  return this;
};
  ...
  while (n--) {
    (function(worker){
      ...
      worker.on('job complete', function(job){
        self.client.incrby(redis.key('q:stats:work-time'), job.duration);
      });
...
Queue.prototype.types = function(fn){
  this.client.smembers(redis.key('q:job:types'), fn);
  return this;
};
...
Queue.prototype.state = function(state, fn){
  this.client.zrange(redis.key('q:jobs:' + state), 0, -1, fn);
  return this;
};
...
Queue.prototype.workTime = function(fn){
  this.client.get(redis.key('q:stats:work-time'), function(err, n){
...
Queue.prototype.card = function(state, fn){
  this.client.zcard(redis.key('q:jobs:' + state), fn);
  return this;
};
