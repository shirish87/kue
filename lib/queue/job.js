...
function getSearch() {
  ...
  return search = reds.createSearch(redis.key('q:search'));
};
...
exports.range = function(from, to, order, fn){
  redis.client().zrange(redis.key('q:jobs'), from, to, get(fn, order));
};
...
exports.rangeByState = function(state, from, to, order, fn){
  redis.client().zrange(redis.key('q:jobs:' + state), from, to, get(fn, order));
};
...
exports.rangeByType = function(type, state, from, to, order, fn){
  redis.client().zrange(redis.key('q:jobs:' + type + ':' + state), from, to, get(fn, order));
};
...
exports.get = function(id, fn){
  ...
  job.id = id;
  client.hgetall(redis.key('q:job:' + job.id), function(err, hash){
    if (err) return fn(err);
...
exports.log = function(id, fn){
  redis.client().lrange(redis.key('q:job:' + id + ':log'), 0, -1, fn);
};
...
Job.prototype.log = function(str){
  ...
  this.client.rpush(redis.key('q:job:' + this.id + ':log'), str);
...
Job.prototype.set = function(key, val, fn){
  this.client.hset(redis.key('q:job:' + this.id), key, val, fn || noop);
  return this;
};
...
Job.prototype.get = function(key, fn){
  this.client.hget(redis.key('q:job:' + this.id), key, fn || noop);
  return this;
};
...
Job.prototype.remove = function(fn){
  ...
  this.client.del(redis.key('q:job:' + this.id + ':log'), noop);
  this.client.del(redis.key('q:job:' + this.id), fn || noop);
  return this;
};
...
Job.prototype.removeState = function(fn){
  var client = this.client
    , state = this._state;
  client.zrem(redis.key('q:jobs'), this.id);
  client.zrem(redis.key('q:jobs:' + state), this.id);
  client.zrem(redis.key('q:jobs:' + this.type + ':' + state), this.id);
  return this;
};
...
Job.prototype.state = function(state){
  ...
  client.zadd(redis.key('q:jobs'), this._priority, this.id);
  client.zadd(redis.key('q:jobs:' + state), this._priority, this.id);
  client.zadd(redis.key('q:jobs:' + this.type + ':' + state), this._priority, this.id);
  // increase available jobs, used by Worker#getJob()
  if ('inactive' == state) client.lpush(redis.key('q:' + this.type + ':jobs'), 1);
  this.set('updated_at', Date.now());
  return this;
};
...
Job.prototype.save = function(fn){
  ...
  // incr id
  client.incr(redis.key('q:ids'), function(err, id){
    if (err) return fn(err);
    var key = redis.key('q:job:' + id);
    self.id = id;
    self._state = self._state || 'inactive';
    if (max) client.hset(key, 'max_attempts', max);
    client.sadd(redis.key('q:job:types'), self.type);
